package mafuyu33.vrgettingoverit.util;
...
public class Vec3History {
    ...
    public Vec3d netMovement(double seconds) {
        ...
        return vec3history$entry != null && vec3history$entry1 != null ? vec3history$entry.data.subtract(vec3history$entry1.data) : new Vec3d(0.0D, 0.0D, 0.0D);
    }
    ...
    public Vec3d averagePosition(double seconds) {
        ...
        for (j = 0; listiterator.hasPrevious(); vec3d = vec3d.add(vec3history$entry.data)) {
        ...
        return j == 0 ? vec3d : vec3d.multiply(1.0D / (double) j);
    }

    private class entry {
        public long ts = Util.getMeasuringTimeMs();
        public Vec3d data;
