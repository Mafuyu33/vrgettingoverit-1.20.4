...
public class VRPlugin {
    ...
    private static boolean checkVRAPIVersion(IVRAPI api) {
        ...
        try {
            ...
            VRGettingOverIt.LOGGER.error("unknown error while checking version for vrapi", ex);
            return false;
        }
        ...
        if (majorVersion != COMPATIBLE_VRAPI_VERSION[0]) {
            VRGettingOverIt.LOGGER.warn("found vrapi, but its major version is {}, which is not be compatible with {} ({})", majorVersion, COMPATIBLE_VRAPI_VERSION[0], COMPATIBLE_VRAPI_VERSION_STRING);
            return false;
        }
        ...
        if (minorVersion > COMPATIBLE_VRAPI_VERSION[1]) {
            //副版本号更大只警告
            //这也许会有一点问题，但是我觉得可以接受
            VRGettingOverIt.LOGGER.warn("found vrapi, but its minor version is {}, which may not be compatible with {} ({}). Use it at your onw risk!", minorVersion, COMPATIBLE_VRAPI_VERSION[1], COMPATIBLE_VRAPI_VERSION_STRING);
        } else if (minorVersion < COMPATIBLE_VRAPI_VERSION[1]) {
            //副版本号更小，不兼容
            VRGettingOverIt.LOGGER.warn("found vrapi, but its minor version is {}, which is not be compatible with {} ({}).", minorVersion, COMPATIBLE_VRAPI_VERSION[1], COMPATIBLE_VRAPI_VERSION_STRING);
            return false;
        } else {
            // 副版本号一致，检查补充版本号
            if (patchVersion < COMPATIBLE_VRAPI_VERSION[2]){
                //补充版本号更小，不兼容
                VRGettingOverIt.LOGGER.warn("found vrapi, but its patch version is {}, which is not be compatible with {} ({}).", patchVersion, COMPATIBLE_VRAPI_VERSION[2], COMPATIBLE_VRAPI_VERSION_STRING);
                return false;
            }
        }
